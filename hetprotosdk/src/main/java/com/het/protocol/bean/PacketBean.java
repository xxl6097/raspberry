...
//        data = new byte[0];
    ...
    public static void main(String[] args) {
        ...
        DeviceInfoBean dm = new DeviceInfoBean();
        dm.setDeviceMac("uuxia");
        dm.setCommandType((short) 7);
        packetBean.setBody(body);
        packetBean.setData(data);
        packetBean.setDeviceInfo(dm);
//        System.out.println("------------before---" + packetBean.toString());

        try {
            PacketBean copyOne = (PacketBean) packetBean.deepCopy();
//            System.out.println("------------copyyy---" + copyOne.toString());
            byte[] body1 = copyOne.getBody();
            body1[0] = 100;
            DeviceInfoBean dd = copyOne.getDeviceInfo();
...
//            System.out.println("------------chcopy---" + copyOne.toString());
    ...
    public String toString() {
        return "PacketBean{" +
                "data=" + ProtoUtils.toHexString(data) +
                ", body=" + ProtoUtils.toHexString(body) +
                ", userKey=" + ProtoUtils.toHexString(userKey) +
                ", updateFlag=" + ProtoUtils.toHexString(updateFlag) +
    ...
    public void resetCommendType() {
        if (data != null) {
            short cmd = 0;
            if (isOpenProtocol) {
                cmd = (short) ProtoUtils.getCommandForOpen(data);
            } else {
                cmd = (short) ProtoUtils.getCommandNew(data);
            }
