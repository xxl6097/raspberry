...
public class IpUtils {
    ...
    public static String getBroadcastAddress(Context ctx) {
        try {
            ...
            Logc.i(Logc.HetLogRecordTag.INFO_WIFI, "get broadcastip：" + broad);
            return broad;
        } catch (SocketException e) {
            e.printStackTrace();
            Logc.e(Logc.HetLogRecordTag.WIFI_EX_LOG, "get broadcastip failed: " + e.getMessage());
        }
        return null;
    }
            ...
            if (!ni.isLoopback()) {
                for (InterfaceAddress interfaceAddress : ni.getInterfaceAddresses()) {
                    if (interfaceAddress.getBroadcast() != null) {
                        String broad = interfaceAddress.getBroadcast().toString().substring(1);
                        Logc.i(Logc.HetLogRecordTag.INFO_WIFI, "广播地址==" + broad);
                        return broad;
                    }
                }
            }
