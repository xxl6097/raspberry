...
public class DeviceInfoBean implements Serializable {
    ...
    /**
     * 绑定状态
     * 如果收到onBindFinish(HashMap<String, DeviceInfoBean> bindMap)回调，且bindMap大小为0，需要调用HTTP接口查询绑定状态，根据最终查询结果判断绑定状态；
...
//    }
    ...
    public void setDeviceMacArray(byte[] macAdddrArray) {
        if (macAdddrArray == null || macAdddrArray.length <= 0)
            return;
        deviceMac = ProtoUtils.byteToMac(macAdddrArray);
...
//        System.out.println("====key=setFrameSN="+frameSN+"  frameNo="+frameNo);
    ...
    public byte[] getDeviceMacToByte() {
        if (!ProtoUtils.isNull(getDeviceMac())) {
            if (ProtoUtils.hexStringToBytes(getDeviceMac()) != null && ProtoUtils.hexStringToBytes(getDeviceMac()).length == 6)
                return ProtoUtils.hexStringToBytes(getDeviceMac());
        }
        return new byte[6];
    }
    ...
    public byte[] getDeviceMacArray() {
        if (deviceMac == null || deviceMac.equals(""))
            return null;
        byte[] nMac = ProtoUtils.hexStringToBytes(deviceMac);
        if (nMac == null)
            return null;
        //System.out.println("@@@@@@@@@@@@@@@@@getDeviceMacArray "+ProtoUtils.byteToMac(nMac));
        return nMac;
    }
